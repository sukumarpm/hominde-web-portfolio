...
function SecurityAppMockup() {
  return (
    <div
      className="rounded-2xl overflow-hidden flex-shrink-0 theme-transition"
      style={{
        width: "340px",
        background: "var(--card)",
        border: "1px solid rgba(5,150,105,0.35)",
        boxShadow: "0 8px 32px rgba(5,150,105,0.12), var(--card-shadow)",
      }}
    >
      {/* App header */}
      <div
        className="px-4 py-3 flex items-center justify-between border-b theme-transition"
        style={{ background: "rgba(5,150,105,0.08)", borderColor: "rgba(5,150,105,0.2)" }}
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[9px]" style={{ color: "#059669" }}>Gate 1 — Main Entrance</p>
          </div>
        </div>
      ...
      {/* Stats row */}
      <div className="grid grid-cols-3 gap-2 p-3 theme-transition" style={{ background: "var(--card)" }}>
        {[
          { label:"Inside Now",    value:"8",  color:"#2563EB" },
          { label:"Entries Today", value:"34", color:"#059669" },
          { label:"Exits Today",   value:"26", color:"#D97706" },
        ].map((s) => (
          <div
            key={s.label}
            className="p-2 rounded-xl text-center theme-transition"
            style={{ background:`${s.color}10`, border:`1px solid ${s.color}25` }}
          >
            <p className="font-mono-data text-base font-bold" style={{ color:s.color }}>{s.value}</p>
            <p className="text-[8px] mt-0.5" style={{ color:"var(--text-2)" }}>{s.label}</p>
          </div>
        ))}
      </div>

      {/* Scan area */}
      <div className="px-3 pb-3 theme-transition" style={{ background: "var(--card)" }}>
        <div
          className="rounded-xl p-3 flex items-center gap-3 mb-3 theme-transition"
          style={{ background:"var(--blue-bg)", border:"1px solid var(--blue-border)" }}
        >
          {/* QR */}
          <div
            className="w-16 h-16 rounded-lg flex items-center justify-center flex-shrink-0 theme-transition"
          ...
          <div>
            ...
            <p className="text-[9px]" style={{ color:"var(--text-2)" }}>Visiting: Flat B-204</p>
            <p className="text-[9px]" style={{ color:"var(--text-2)" }}>Expected: 2:00 PM</p>
          </div>
        </div>
        ...
        >
          <p className="text-[9px] font-semibold mb-2" style={{ color:"var(--text-3)" }}>RECENT LOG</p>
          {[
            ...
            >
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ background:log.color }} />
                <div>
                  <p className="text-[9px] font-medium" style={{ color:"var(--text-2)" }}>{log.name}</p>
                  <p className="text-[8px]" style={{ color:"var(--text-3)" }}>{log.flat}</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-[9px] font-semibold" style={{ color:log.color }}>{log.action}</p>
                <p className="text-[8px]" style={{ color:"var(--text-3)" }}>{log.time}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Bottom nav */}
      <div
        className="flex items-center justify-around px-4 py-2.5 border-t theme-transition"
      ...
      >
        {[
          ...
          <div key={item.label} className="flex flex-col items-center gap-0.5">
            <div
              className="w-7 h-7 rounded-lg flex items-center justify-center text-sm theme-transition"
              style={{ background: item.active ? "rgba(5,150,105,0.15)" : "transparent" }}
            >
              {item.icon}
            </div>
            <span className="text-[7px]" style={{ color: item.active ? "#059669" : "var(--text-3)" }}>
              {item.label}
            </span>
          </div>
        ))}
    ...
    >
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-16">
          <p className="text-sm font-semibold tracking-wider uppercase mb-4" style={{ color:"#059669" }}>
            Security Platform
          </p>
          ...
          <p className="text-lg max-w-xl mx-auto" style={{ color:"var(--text-2)" }}>
            Give your gate staff everything they need — QR scanning, real-time visitor tracking,
            attendance and emergency response — in one purpose-built interface.
          </p>
        </div>

        <div className="flex flex-col lg:flex-row items-start gap-12 justify-center">
          <div className="flex-shrink-0 flex justify-center w-full lg:w-auto animate-float" style={{ animationDuration:"5.5s" }}>
            <SecurityAppMockup />
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {securityModules.map((mod) => (
                <div
                  ...
                  onMouseEnter={(e) => {
                    (e.currentTarget as HTMLElement).style.background = "rgba(5,150,105,0.1)";
                    (e.currentTarget as HTMLElement).style.borderColor = "rgba(5,150,105,0.35)";
                    (e.currentTarget as HTMLElement).style.transform = "translateY(-2px)";
                  }}
                  onMouseLeave={(e) => {
                    (e.currentTarget as HTMLElement).style.background = "rgba(5,150,105,0.05)";
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-semibold mb-0.5" style={{ color:"var(--text-1)" }}>{mod.label}</p>
                    <p className="text-xs leading-snug" style={{ color:"var(--text-2)" }}>{mod.desc}</p>
                  </div>
                </div>
              ))}
            </div>
